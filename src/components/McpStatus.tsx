...
export function McpStatus() {
  ...
  return (
    <div className="relative">
      ...
      {showDetails && info && (
        <div className="absolute right-0 top-full mt-1 bg-bg-secondary border border-border rounded-md shadow-lg p-4 z-50 w-80 animate-slide-down">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-text-primary">MCP Server</span>
            <span className="badge badge-green text-[10px]">运行中</span>
          </div>
          ...
          <div className="mt-3 pt-3 border-t border-border">
            ...
            <pre className="bg-bg-primary rounded-md p-2 text-xs text-text-primary overflow-x-auto font-mono">
              {configJson}
            </pre>
          </div>
          ...
          <div className="mt-2 pt-2 border-t border-border">
            ...
            <div className="flex flex-wrap gap-1">
              {info.tools.map((tool) => (
                <span key={tool} className="text-[10px] bg-bg-primary text-text-muted px-1.5 py-0.5 rounded">
                  {tool}
                </span>
              ))}
            </div>
